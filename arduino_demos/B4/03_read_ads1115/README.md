# B4 Arduino Example 03: Read ADS1115 Analog Input Ports Value

## Description
This example demonstrates how to read voltage values from all four channels (A0, A1, A2, A3) of the ADS1115 ADC module on the B4 board. The readings are printed to the Serial Monitor in millivolts (mV).

## File Structure
- `src/B4_03_read_ads1115.ino` – Arduino source code for customization and compilation.
- `precompiled/B4_03_read_ads1115.bin` – Precompiled firmware binary for ESP32-S3, ready for direct flashing.
...
3. **Steps**:
   - Open `src/B4_03_read_ads1115.ino` in Arduino IDE.
...
## Important Notes
- **I2C Address**: The code uses address `0x49` (`ADS1115_IIC_ADDRESS0`). Verify your hardware configuration – the B4 pin definition post mentions address `0x48`. If your board uses `0x48`, change the line to:
  ```cpp
