# B4 Arduino Example 06: How to Use SD Card

## Description
This example demonstrates basic file operations on an SD card using the B4 board. It shows how to initialize the SD card, perform read/write/append/delete operations, and test file I/O performance. All results are printed to the Serial Monitor.

## File Structure
- `src/B4_06_sd_card_operations.ino` – Arduino source code for customization and compilation.
- `precompiled/B4_06_sd_card_operations.bin` – Precompiled firmware binary for ESP32-S3, ready for direct flashing.
...
3. **Steps**:
   - Open `src/B4_06_sd_card_operations.ino` in Arduino IDE.
...
## Important Notes
- **Pin Configuration**: The code uses the following B4 SD Card pins:
  - SCK  = GPIO11
  - MISO = GPIO12
  - MOSI = GPIO10
  - CS   = GPIO9
...
- **Power Supply**: SD cards can draw significant current during writes. Ensure your B4 is powered adequately (not just via USB from a weak source) to avoid corruption.
...
- **Refer to Pin Definitions**: For detailed pinout information, see the [B4 pin definition](../../../pin_definitions/B4_pin_definition.md).