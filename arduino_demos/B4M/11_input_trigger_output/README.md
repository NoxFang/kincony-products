# B4M Arduino Example 11: Digital Input Triggers Output Directly

## Description
This example demonstrates a direct linkage between the 4 digital inputs and the 4 relay outputs on the B4M board using the PCF8574 I/O expander. It continuously reads the state of input pins (P0-P3) and immediately updates the corresponding output pins (P4-P7). When an input pin is **LOW**, the associated relay output is set to **LOW** (turning the relay ON).

## File Structure
- `src/B4M_11_input_trigger_output.ino` – Arduino source code for customization and compilation.
- `precompiled/B4M_11_input_trigger_output.bin` – Precompiled firmware binary for ESP32-S3, ready for direct flashing.
...
- **PCF8574 Address**: The I2C address is set to `0x24`, which is correct for the B4M's PCF8574.
...
- Pull-up Resistors: The inputs (P0-P3) on the B4M typically have pull-up resistors. This means they read HIGH when no connection is made. To trigger an input, connect it to GND.
...
## References
1. For detailed pinout information, see the B4M pin definition.
